...
public extension PhenixCore.Room {
    struct Configuration: CustomStringConvertible {
        ...
        /// If set to `true`, then **PhenixCore** automatically subscribes to each ``PhenixCore/PhenixCore/Member`` media,
        /// when this member appears in the ``PhenixCore/PhenixCore/members`` list.
        ///
        /// Default value is `true`.
        ///
        /// If set to `false`, then to subscribe to all ``PhenixCore/PhenixCore/Member`` media in the ``PhenixCore/PhenixCore/members`` list,
        /// you must call ``PhenixCore/PhenixCore/subscribeToRoomMembers()``.
