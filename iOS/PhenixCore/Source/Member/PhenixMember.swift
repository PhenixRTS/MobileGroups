...
public extension PhenixCore {
    final class Member: Identifiable {
        ...
        /// A phenix member's internal state publisher, which does not publish changes on the provided publisher's queue.
        ///
        /// Use this publisher inside the PhenixCore. Be aware that it is triggered on PhenixSdk's insternal queue.
        /// It is like that because we need to be aware of the property change as fast as possible. Moving it to the
        /// PhenixCore's queue will cause late updates.
        var phenixStatePublisher: AnyPublisher<PhenixMemberState, Never> { phenixMember.statePublisher }
        var phenixStreamsPublisher: AnyPublisher<[PhenixStream], Never> { phenixMember.streamsPublisher }
        ...
        public var id: String {
            phenixMember.getSessionId()
        }

        public var role: Role {
            Role(phenixMember.getObservableRole().getValueOrDefault()) ?? .audience
        }

        public var name: String {
            String(phenixMember.getObservableScreenName().getValueOrDefault())
        }

        public var state: State {
            State(phenixMember.getObservableState().getValueOrDefault()) ?? .active
        }
