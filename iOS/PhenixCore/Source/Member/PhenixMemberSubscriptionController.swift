        ...
        func start() {
            ...
            memberConnectionStateCancellable = member.connectionStatePublisherInternal
                ...
                .sink { [weak self] state in
                    ...
                    case .pending:
                        self.member.dropLastSecondaryVideoFrame()
                        self.stopConnectionStateWatchdog()
            ...
            memberStreamListCancellable = member.phenixStreamsPublisher
                .receive(on: queue)
                .sink { [weak self] streams in
                    if streams.isEmpty {
                        /*
                         Situations, when the member does not have any streams
                         happens when the member have stopped publishing.
                         That also counts as ended stream and the member connection
                         isn't active anymore.
                         We are using `.custom` reason here to indicate that
                         this was set manually, not from the PhenixSdk.
                         */
                        self?.onStreamEnded(reason: .custom)
                    } else {
